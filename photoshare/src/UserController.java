...
public class UserController {
	...
	private static final String SEARCH_USER_BY_PHOTO = "SELECT * FROM Users u WHERE u.user_id IN (SELECT a.user_id FROM Albums a INNER JOIN Pictures p ON p.album_id = a.album_id WHERE p.picture_id = ?) LIMIT 1;";
	...
	public static User getUserByPhotoId(int photo_id) {
		PreparedStatement stmt = null;
		Connection conn = null;
		ResultSet rs = null;
		Picture picture = null;
		...
		try {
			conn = DbConnection.getConnection();
			stmt = conn.prepareStatement(SEARCH_USER_BY_PHOTO);
